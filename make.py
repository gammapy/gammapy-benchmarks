...
AVAILABLE_BENCHMARKS = {
    'iact-basic-image-estimator': 'benchmarks/iact_basic_image_estimator.py',
}

MONITOR_OPTIONS = {'duration': None,
                   'interval': 0.5,
...
@click.option('--tag', help='Assign a tag to the benchmark run, so results will'
...
def run_benchmarks(benchmarks, tag):
    ...
    for benchmark in benchmarks:
        ...
        results_folder = THIS_REPO / 'results/{benchmark}/{version}/{tag}'.format(**folder_)
        ...
        run_single_benchmark(benchmark, logfile=str(results_filename),
                             plot=str(plot_filename), **MONITOR_OPTIONS)
