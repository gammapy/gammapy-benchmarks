...
# MapEvaluator import can vary slightly across Gammapy versions
try:
    from gammapy.datasets import MapEvaluator
except Exception:
...
def _make_common_objects():
    ...
    return evaluator, center, geom


def _set_psf_kernel_size(evaluator, geom, *, kernel_size: int):
    ...
    evaluator.psf = PSFKernel.from_gauss(
...
def _prepare_mode_b_image_broadcast(evaluator, center, geom):
...
def _report_mode(mode_name, npred_shape, kernel_shape, cpu_res, gpu_res):
    ...
    cmp_stats = _compare_arrays(cpu_res.out_np, gpu_res.out_np, compare_dtype=COMPARE_DTYPE)
    ...
    if not cmp_stats.get("shape_match", False):
        ...
        print(f"    global_scale   = {cmp_stats['global_scale']:.6e}, eps_used={cmp_stats['eps_used']:.6e}")
...
def _print_sweep_table(title, rows):
    ...
    print(f"{'kernel_req':>10}  {'kernel_act':>10}  {'CPU ms':>10}  {'GPU ms':>10}  {'speedup':>10}")
    for r in rows:
        print(
...
def main():
    ...
    # Mode A: cube (E,Y,X)
    # ---------------------
    evaluator_a, center, geom = _make_common_objects()
    ...
    # ------------------------------------------------
    evaluator_b, center, geom = _make_common_objects()
    npred_b, geom_k_b = _prepare_mode_b_image_broadcast(evaluator_b, center, geom)
    ...
    # ---- Sweep for Mode A ----
    sweep_rows_a = []
    evaluator_sweep_a, center, geom = _make_common_objects()
    npred_sweep_a = _prepare_mode_a_cube(evaluator_sweep_a)

    for kreq in KERNEL_SIZES:
        ky, kx = _set_psf_kernel_size(evaluator_sweep_a, geom, kernel_size=kreq)
    ...
    # ---- Sweep for Mode B ----
    sweep_rows_b = []
    evaluator_sweep_b, center, geom = _make_common_objects()
    npred_sweep_b, geom_k_sweep = _prepare_mode_b_image_broadcast(
        evaluator_sweep_b, center, geom
    )

    for kreq in KERNEL_SIZES:
        ky, kx = _set_psf_kernel_size(evaluator_sweep_b, geom_k_sweep, kernel_size=kreq)
        cpu = _run_apply_psf_timed(
            evaluator_sweep_b,
            npred_sweep_b,
            force_cpu=True,
