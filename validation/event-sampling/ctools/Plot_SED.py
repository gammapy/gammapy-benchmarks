...
y_unit = u.Unit(y_u)
min_y_ratio = 36.0  # this ensures at least one major tick
...
def main():
    ...
    if E_bf is not None:
        ...
        plt.fill_between(
            x=E_bf.to(x_u) / x_unit,
            y1=Fmin_bf.to(y_u) / y_unit,
            y2=Fmax_bf.to(y_u) / y_unit,
            color="blue",
            alpha=0.5,
            zorder=1,
        )  # Bottom
...
def get_dNdE(src):
    ...
    if spec_model == "PowerLaw":
        ...
        return None, None
    E_TeV = np.linspace(
        Emin.to("TeV") / u.Unit("TeV"), Emax.to("TeV") / u.Unit("TeV"), 100
    )
...
def get_PowerLaw_model(XML_spectrum):
    ...
    for idx, item in enumerate(XML_spectrum[:]):
        ...
        if par_name == "Prefactor":
            par_array[0] = XML_value * 1.0e6
        elif par_name == "Scale" or par_name == "PivotEnergy":
            par_array[1] = XML_value / 1.0e6
        elif par_name == "Index":
            par_array[2] = -1.0 * XML_value
    print(
        "Parameters: Prefactor=%g, Scale=%e, Index=%f"
...
def get_PowerLaw_model_error(XML_spectrum):
    ...
    for idx, item in enumerate(XML_spectrum[:]):
        ...
        if par_name == "Prefactor":
            par_array[0] = XML_value * 1.0e6
            par_array[2] = (
                float(item.attrib.get("error"))
                * float(item.attrib.get("scale"))
                * 1.0e6
            )
        elif par_name == "Index":
            par_array[1] = -1.0 * XML_value
            par_array[3] = (
                -1.0 * float(item.attrib.get("error")) * float(item.attrib.get("scale"))
            )
    return par_array
...
def get_PowerLaw2_model(XML_spectrum):
    ...
    for idx, item in enumerate(XML_spectrum[:]):
        ...
        if par_name == "Integral":
            ...
            X_max = XML_value / 1.0e6
        elif par_name == "LowerLimit":
            X_min = XML_value / 1.0e6
        elif par_name == "Index":
            alpha = XML_value
    if alpha == -1:
        return None
    amplitude = (
        integral * (1.0 + alpha) / (X_max ** (1.0 + alpha) - X_min ** (1.0 + alpha))
    )
    par_array = [amplitude, 1.0, -1.0 * alpha]
    print(
        "Parameters: Prefactor=%g, Scale=%e, Index=%f"
        % (par_array[0], par_array[1], par_array[2])
    )
...
def get_ExpCutoff_model(XML_spectrum):
    ...
    for idx, item in enumerate(XML_spectrum[:]):
        ...
        if par_name == "Prefactor":
            ...
            par_array[3] = XML_value / 1.0e6
    print(
        "Parameters: Prefactor=%g, PivotEnergy=%e, Index=%f, CutoffEnergy=%e"
...
def get_LogParabola_model(XML_spectrum):
    ...
    for idx, item in enumerate(XML_spectrum[:]):
        ...
        if par_name == "Prefactor":
            ...
            par_array[3] = -1.0 * XML_value
    print(
        "Parameters: Prefactor=%g, Scale=%e, Index=%f, Beta=%f"
        % (par_array[0], par_array[1], par_array[2], par_array[3])
    )
...
def get_BrokenPowerLaw_model(XML_spectrum):
    ...
    for idx, item in enumerate(XML_spectrum[:]):
        ...
        if par_name == "Prefactor":
            ...
            par_array[3] = -1.0 * XML_value
    print(
        "Parameters: Prefactor=%g, BreakValue=%e, Index1=%f, Index2=%f "
        % (par_array[0], par_array[1], par_array[2], par_array[3])
    )
...
def parse_command_line():
    ...
    Parser.add_argument(
        "--butterfly",
        "-b",
        default="butterfly.txt",
        help="ASCII butterfly file (output of ctbutterfly)",
    )
    Parser.add_argument(
        "--spec",
        "-s",
        default="spec.fits",
        help="FITS binned spectrum (output of csspec)",
    )
    Parser.add_argument(
        "--threshold",
        "-t",
        type=float,
        default=3.5,
        help="detection threshold in the FITS binned spectrum (default 3.5 sigma)",
    )
    Parser.add_argument("--model", "-m", default="model.xml", help="XML model file")
    Parser.add_argument(
        "--name", "-n", default="source", help="source name in the XML model file"
    )
    Parser.add_argument(
        "--outfile",
        "-o",
        default=None,
        help="output PNG image file name (default SED_$NAME$_v1.png)",
    )
